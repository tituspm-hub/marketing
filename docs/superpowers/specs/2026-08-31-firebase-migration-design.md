...
system runs entirely on free tiers; super-admins increased from two to three; the build
is developed against the Firebase emulators and provisioned against a real project last.
...
3. Three permanent super-admins (Yash, Titus, Gebin) who cannot be demoted or removed
   by anyone.
...
The entire application is built and tested against the **Firebase Emulator Suite** using
the project id `demo-hire3x`. Firebase treats any project id beginning with `demo-` as
offline-only: it never contacts Google, needs no login, and cannot touch production data.

A real Firebase project and a Vercel deployment are created **last**, as the final task,
at which point going live is a change of environment variables rather than a change of
code. Nothing in the codebase distinguishes the two environments except `.env.local`.

### 4.3 The central decision — document granularity
...
The three super-admins are created with **deterministic UIDs** — `sa_yash`, `sa_titus`,
`sa_gebin` — set explicitly by the bootstrap script rather than generated by Firebase.
The list is therefore a fixed literal that is identical in the emulator and in
production, and never needs substituting after a deploy.

Two independent mechanisms enforce it, both server-side:

1. `firestore.rules` contains the literal list in `superAdmins()`.
2. `src/shared/roles.js` contains the same literal list and is imported by both the
   browser bundle and the Vercel backend. Every API route rejects any operation whose
   *target* UID appears in it.

A unit test parses `firestore.rules`, extracts its list, and asserts it equals
`SUPER_ADMIN_UIDS` exactly. The two definitions cannot silently diverge.
...
isSuper()     request.auth.uid in ['sa_yash', 'sa_titus', 'sa_gebin']
...
Super-admins cannot be created through the app, since creating one would require an
admin. `scripts/bootstrap.mjs` creates all three with their fixed UIDs, sets their
`superadmin` custom claims, writes their `/users` documents, seeds `/settings/app`, and
prints the temporary passwords.

It runs in two modes. Against the emulator it needs no credentials at all — it detects
`FIRESTORE_EMULATOR_HOST` and connects locally. Against production it reads a
service-account key. The script is idempotent: re-running repairs claims and documents
without duplicating accounts.

For production the service-account key is base64-encoded into the Vercel environment
variable `FIREBASE_SERVICE_ACCOUNT` and the local copy deleted. The key is never
...
The team is fixed and needs nothing further:

| Username | Name | Role |
| --- | --- | --- |
| `yash` | Yash | superadmin |
| `titus` | Titus | superadmin |
| `gebin` | Gebin | superadmin |
| `shijin` | Shijin | member |
| `madesh` | Madesh | member |

Everything before the final task is built against the emulator and needs **no external
accounts**. Provisioning is deferred to the last task, where the user supplies:

1. A Firebase project on the free **Spark** plan with Email/Password auth and Firestore
   enabled, and the web app `firebaseConfig` block.
2. A Vercel project linked to this repository, on the free Hobby plan.
3. A Firebase service-account key, base64-encoded into the Vercel environment variable
   `FIREBASE_SERVICE_ACCOUNT`.

No UID substitution is needed at any point; the super-admin UIDs are fixed literals
(§7.2).
...
1. **Foundation.** Emulator config, auth provider, login, forced password change, route
   guards, bootstrap script, rules v1 with tests. No real Firebase project yet.
...
7. **Hardening.** Security review, full failure-point sweep against §10.
8. **Provisioning.** Create the real Firebase project, deploy rules, run bootstrap
   against production, link Vercel, deploy. Configuration only — no code changes.
